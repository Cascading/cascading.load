...
public class Pipeline extends Load
  ...
  public Flow createFlow() throws Exception
    ...
    for( int i = 0; i < 50; i++ )
      {
      pipe = new Each( pipe, new Fields( "hash" ), new Identity( new Fields( Long.TYPE ) ), Fields.ALL );
      pipe = new Each( pipe, new Fields( "sum1" ), new Identity( new Fields( Long.TYPE ) ), Fields.ALL );
