...
it_parses_the_latest_load_location () {
  testing_url="http://files.cascading.org/load/2.1/load-latest.tgz"
...
it_complains_if_curl_fails_to_fetch_latest () {
  ...
  module_exit () {
    [ "$*" = "Cannot get latest Load from http://files.cascading.org/load/2.1/latest.txt" ] && tested=1
  }
  cl_update_create_temp () {
    [ "$tested" = "1" ] && tested=2
  }
...
it_allows_a_version_specifier () {
  ...
  test "$cl_update_latest" = "http://files.cascading.org/load/2.1/load-latest.tgz"

  route_perform update --version more_latest
  test "$cl_update_latest" = "http://files.cascading.org/load/2.1/load-more_latest.tgz"

  route_perform update --version=super_latest
  test "$cl_update_latest" = "http://files.cascading.org/load/2.1/load-super_latest.tgz"
}
